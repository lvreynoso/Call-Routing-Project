...
class BinaryTreeNode(object):
    ...
    def __init__(self, key):
        """Initialize this binary tree node with the given key."""
        self.key = key
        self.data = {}
        self.left = None
        self.right = None
    ...
    def __repr__(self):
        ...
        return 'BinaryTreeNode({!r}, {!r})'.format(self.key, self.data)

    def is_leaf(self):
        """Return True if this node is a leaf (has no children)."""
        # Check if both left child and right child have no key
        return self.left is None and self.right is None

    def is_branch(self):
        """Return True if this node is a branch (has at least one child)."""
        # Check if either left child or right child has a key
    ...
    def height(self):
        ...
        # Check if left child has a key and if so calculate its height
        left_height = 0
        if self.left:
            left_height = 1 + self.left.height()
        # Check if right child has a key and if so calculate its height
        right_height = 0
        if self.right:
...
class BinarySearchTree(object):
    ...
    def height(self):
        ...
        # Check if root node has a key and if so calculate its height
    ...
    def delete(self, item):
        """Remove given item from this tree, if present, or raise keyError.
